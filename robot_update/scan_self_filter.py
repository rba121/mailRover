...
class ScanSelfFilterNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('self_filter_x_min_m', -0.23)
        self.declare_parameter('self_filter_x_max_m', 0.23)
        self.declare_parameter('self_filter_y_min_m', -0.23)
        self.declare_parameter('self_filter_y_max_m', 0.23)

        input_topic = self.get_parameter('input_scan_topic').value
        output_topic = self.get_parameter('output_scan_topic').value
        self.enabled = bool(self.get_parameter('enabled').value)
        self.laser_x_offset_m = float(
            self.get_parameter('laser_x_offset_m').value
        )
        self.laser_y_offset_m = float(
            self.get_parameter('laser_y_offset_m').value
        )
        ...
        # SensorDataQoS is compatible with the best-effort QoS commonly used
        # by ROS 2 lidar drivers.
    ...
    def is_self_hit(self, reading, angle, range_min, range_max):
        if (
            ...
            or reading < range_min
            or reading > range_max
        ):
            return False

        base_x = (
            self.laser_x_offset_m
            + reading * math.cos(angle)
        )
        base_y = (
            self.laser_y_offset_m
            + reading * math.sin(angle)
        )

        return (
            self.self_filter_x_min_m
            <= base_x
            <= self.self_filter_x_max_m
            and self.self_filter_y_min_m
            <= base_y
            <= self.self_filter_y_max_m
        )
