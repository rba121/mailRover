...
class EncoderOdometryNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('track_width_m', 0.3937)
        ...
        self.left_direction = float(
            self.get_parameter('left_direction').value
        )
        self.right_direction = float(
            self.get_parameter('right_direction').value
        )
        ...
        self.get_logger().info(
            ...
            f'track_width_m={self.track_width_m:.4f}, '
    ...
    def publish_odometry(self):
        ...
        self.left_encoder.poll()
        self.right_encoder.poll()
        ...
        meters_per_tick = (
            ...
            * math.pi
            * self.wheel_radius_m
            / self.ticks_per_wheel_rev
        )
        delta_left_ticks = (
            left_count - self.last_left_count
        ) * self.left_direction
        delta_right_ticks = (
            right_count - self.last_right_count
        ...
        left_spike = left_speed > self.max_wheel_speed_mps
        right_spike = right_speed > self.max_wheel_speed_mps

        # Reject the complete sample if either encoder is implausible.
        # Accepting only one wheel would create false odometry rotation.
        if left_spike or right_spike:
            self.get_logger().warn(
                'Ignoring encoder sample: '
                f'left={left_speed:.2f} m/s, '
                f'right={right_speed:.2f} m/s'
            )
            delta_left = 0.0
            delta_right = 0.0
