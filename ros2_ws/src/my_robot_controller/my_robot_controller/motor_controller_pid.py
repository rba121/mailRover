...
class PIDController:
    ...
    def compute(self, error, dt):
        ...
        self.integral = max(
            -self.i_clamp,
            min(self.i_clamp, self.integral),
        )
...
class MotorControllerPIDNode(Node):
    def __init__(self):
        ...
        self.declare_parameter(
            ...
            DEFAULT_FEEDBACK_TIMEOUT_SEC,
        )
        self.declare_parameter(
            'pwm_slew_per_sec',
            DEFAULT_PWM_SLEW_PER_SEC,
        )
        ...
        self.pwm_slew_per_sec = float(
            self.get_parameter('pwm_slew_per_sec').value
        )
        ...
        # Encoder odometry publishes physical forward wheel speeds as positive.
        # Convert that feedback into the same signs used by the motor drivers.
        ...
        self.get_logger().info(
            'PID Motor Controller started — '
            f'Kp={kp}, Ki={ki}, Kd={kd}, '
            f'max_pwm={self.max_pwm}, '
            f'max_linear={self.max_linear_mps}, '
    ...
    def cmd_vel_callback(self, msg: Twist):
        ...
        # Calculate physical wheel velocities first.
        left_wheel_mps = linear - angular * (self.track_width_m / 2.0)
        right_wheel_mps = linear + angular * (self.track_width_m / 2.0)

        # Preserve the requested curve while keeping both wheel targets within
        # the configured maximum speed.
        peak_wheel_speed = max(
            abs(left_wheel_mps),
            abs(right_wheel_mps),
        )
        if peak_wheel_speed > self.max_linear_mps > 0.0:
            scale = self.max_linear_mps / peak_wheel_speed
            left_wheel_mps *= scale
            right_wheel_mps *= scale

        # Convert physical wheel directions to motor-driver directions.
        self.target_left_mps = (
            left_wheel_mps * self.left_motor_direction
        )
        self.target_right_mps = (
            right_wheel_mps * self.right_motor_direction
        )
    ...
    def control_loop(self):
        ...
        # Old commands are unsafe, so a command timeout stops immediately.
        elapsed = (now - self.last_cmd_time).nanoseconds / 1e9
        if elapsed > self.command_timeout_sec:
            if not self.command_timeout_logged:
                self.get_logger().warn(
                    'Command timeout; stopping motors'
                )
        ...
        if target_active and (
            ...
            if not self.feedback_timeout_logged:
                self.get_logger().warn(
                    'Encoder feedback timeout; stopping motors'
                )
                self.feedback_timeout_logged = True
            self.stop_motors()
            return

        # Use the measured loop time. Limit it so a delayed callback cannot
        # produce an unusually large PWM step.
        max_step = (
            self.pwm_slew_per_sec
            * self.clamp(dt, 0.0, 0.1)
        )

        # A zero velocity request is a stop command. Do not slew through an
        # obstacle stop.
        if (
            abs(self.target_left_mps) < self.deadband
        ...
        # Feedforward plus PID correction produces an absolute PWM target.
        # It is intentionally not added to the previously applied PWM.
        left_raw_pwm = (
            self.feedforward_pwm(self.target_left_mps)
            + self.left_pid.compute(left_error, dt)
        )
        right_raw_pwm = (
            self.feedforward_pwm(self.target_right_mps)
            + self.right_pid.compute(right_error, dt)
        )
        ...
        # Slew-limit acceleration during normal movement.
        ...
        self.get_logger().debug(
            f'L: target={self.target_left_mps:.2f} '
            f'actual={self.actual_left_mps:.2f} '
            f'err={left_error:.2f} '
            f'pwm={self.applied_left_pwm:.2f} | '
            f'R: target={self.target_right_mps:.2f} '
            f'actual={self.actual_right_mps:.2f} '
            f'err={right_error:.2f} '
            f'pwm={self.applied_right_pwm:.2f}'
        )

    def feedforward_pwm(self, target_mps):
        if abs(target_mps) < self.deadband:
            return 0.0

        if target_mps > 0.0:
            return self.min_effective_pwm
